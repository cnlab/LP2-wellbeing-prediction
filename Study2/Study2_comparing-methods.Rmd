...
pacman::p_load(forecast, effsize,DT, multilevel,tidyverse, stringi,install = T)
...
SWLS_SWLS = t.test(text_errors_SWLS, audio_errors_SWLS, var.equal = F)
...
SWLS_autonomy = t.test(text_errors_autonomy, audio_errors_autonomy, var.equal = F)
...
SWLS_PWB = t.test(text_errors_PWB, audio_errors_PWB, var.equal = F)
...
SWLS_pos_relations = t.test(text_errors_pos_relations, audio_errors_pos_relations, var.equal = F)
...
SWLS_env_master = t.test(text_errors_env_master, audio_errors_env_master, var.equal = F)
...
SWLS_personal = t.test(text_errors_personal, audio_errors_personal, var.equal = F)
...
SWLS_purpose = t.test(text_errors_purpose, audio_errors_purpose, var.equal = F)
...
SWLS_accept = t.test(text_errors_accept, audio_errors_accept, var.equal = F)
...
d_SWLS <- cohen.d(text_errors_SWLS, audio_errors_SWLS, pooled = TRUE)$estimate
d_autonomy <- cohen.d(text_errors_autonomy, audio_errors_autonomy, pooled = TRUE)$estimate
d_PWB <- cohen.d(text_errors_PWB, audio_errors_PWB, pooled = TRUE)$estimate
d_pos_relations <- cohen.d(text_errors_pos_relations, audio_errors_pos_relations, pooled = TRUE)$estimate
d_env_master <- cohen.d(text_errors_env_master, audio_errors_env_master, pooled = TRUE)$estimate
d_personal <- cohen.d(text_errors_personal, audio_errors_personal, pooled = TRUE)$estimate
d_purpose <- cohen.d(text_errors_purpose, audio_errors_purpose, pooled = TRUE)$estimate
d_accept <- cohen.d(text_errors_accept, audio_errors_accept, pooled = TRUE)$estimate
...
results_df$cohens_d <- c(d_SWLS, d_autonomy, d_PWB, d_pos_relations, d_env_master, d_personal, d_purpose, d_accept)

# Round the necessary columns
SWLS_model_errors <- results_df %>% 
  dplyr::select(Measure, mean_text, mean_audio, df, t_statistic, p_value, p_adjusted,cohens_d,conf_int_lower,conf_int_upper) %>% 
  mutate(mean_text = round(as.numeric(mean_text), 2),
         ...
         conf_int_lower = round(as.numeric(conf_int_lower), 3),
         conf_int_upper = round(as.numeric(conf_int_upper), 3),
...
# Add Cohen's d to your results data frame
results_df$cohens_d <- c(d_SWLS, d_autonomy, d_PWB, d_pos_relations, d_env_master, d_personal, d_purpose, d_accept)

# Then do your rounding and select columns including cohens_d
...
```{r}
simulate_cohens_d_power <- function(n_text = 127, n_audio = 95,
                                    d = 0,    # Cohen's d effect size
                                    sd = 1,   # standard deviation in both groups
                                    alpha = 0.05, n_sim = 1e4) {
  
  power <- replicate(n_sim, {
    # Group means separated by d * sd
    mean_text <- 0
    mean_audio <- mean_text - d * sd
    
    # Simulate data
    text_errors <- rnorm(n_text, mean = mean_text, sd = sd)
    audio_errors <- rnorm(n_audio, mean = mean_audio, sd = sd)
    
    # Two-sample t-test
    ttest <- t.test(text_errors, audio_errors, var.equal = FALSE)
...
# Define effect sizes (Cohen's d)
d_values <- seq(0, 0.7, by = 0.05)

powers_d <- sapply(d_values, function(d) simulate_cohens_d_power(d = d))
...
ggplot(power_df, aes(x = cohens_d, y = power)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0.8, linetype = "dashed", color = "red") +
  labs(
    x = "Cohen's d (Standardized Mean Difference)",
    y = "Statistical Power",
    title = "Power to Detect Differences in Methods"
  ) +
  ylim(0, 1) +
  theme_minimal()
