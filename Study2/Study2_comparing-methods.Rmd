...
SWLS_model_errors <- results_df %>% 
  dplyr::select(Measure, mean_text, mean_audio, df, t_statistic, p_value, p_adjusted,cohens_d) %>% 
...
Autonomy_model_errors <- results_df %>% 
  dplyr::select(Measure, mean_text, mean_audio, df, t_statistic, p_value, p_adjusted, cohens_d) %>% 
  mutate(
    mean_text = round(as.numeric(mean_text), 2),
    mean_audio = round(as.numeric(mean_audio), 2),
    df = round(as.numeric(df), 2),
    t_statistic = round(as.numeric(t_statistic), 2),
    p_value = round(as.numeric(p_value), 3),
...
ggplot(power_df, aes(x = cohens_d, y = power)) +
  ...
  labs(
    ...
    y = "Statistical Power",
    title = "Power to Detect Differences in Cohen's d"
  ) +
  ylim(0, 1) +
  theme_minimal()
