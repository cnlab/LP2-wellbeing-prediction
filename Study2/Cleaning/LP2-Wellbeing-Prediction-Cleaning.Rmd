---
title: "LP2 Well-being Prediction"
author: ""
date: "`r Sys.Date()`"
output:
  html_document:
...
survey_name_filter = "LP2 well-being prediction"
...
```{r}
if (!file.exists("../data/well-being-prediction.rds")) {
  ...
  surveys_long = scorequaltrics::get_survey_data(surveysFiltered,
    ...
    gather(item, value, -c("survey_name", "ResponseId", "id", "Reflection_Type",
  ...
  saveRDS(surveys_long, "../data/well-being-prediction.rds")
} else {
  surveys_long = readRDS("../data/well-being-prediction.rds")
}
```
...
surveys_tidy = surveys_long %>%
  filter(!is.na(id)) %>% # remove test responses
  filter(nchar(id) > 1) %>% # remove test responses
  ...
  filter(Progress >80) %>%
  filter( RecordedDate >= "2024-04-04") # double filter out test cases
...
ids = surveys_tidy %>%
  ...
  left_join(., unique(select(surveys_tidy, id, ResponseId)))
...
data_surveys = data %>%
  filter(Finished == 1) %>%
  filter(grepl("Text|Audio", Reflection_Type)) %>%
  filter(!is.na(value)) %>%
  ...
  select(pID, Reflection_Type,survey_name,item, value) 

###can save this to get the item-level responses

###extract the text data
data_text = data_surveys %>% 
  filter(Reflection_Type == "Txext") %>%
...
write.csv(scored,"/Users/stevenmesquiti/Box Sync/CurrentProjects_Penn/LP2/within_person_intervention/data_text/surveys_scored/LP2_Wellbeing-Prediction_Behavioral.csv")
```

## Tidy Phonic Data

```{r}
prompt_text = read.csv("/Users/stevenmesquiti/Box Sync/CurrentProjects_Penn/LP2/within_person_intervention/data/prompt_text_wellbeing_prediction.csv", stringsAsFactors = FALSE) 
...
###make sure to update phonic data
phonic = fs::dir_ls("/Users/stevenmesquiti/Box Sync/CurrentProjects_Penn/LP2/within_person_intervention/data_text/phonic-data/well-being-prediction/", regexp = "*.csv", recurse = TRUE) %>%
  map_dfr(read_events) %>%
  select(Date, responseId, contains("Transcription")) %>%
  filter(Date >= "2024-04-04")  %>% 
  gather(prompt, text, -Date, -responseId) %>% 
  mutate(prompt = ifelse(prompt == "Q2..HILS..Transcription", "Q2..Autonomy..Transcription", prompt)) %>% #rename HILS question... forgot to do this in phonic
  extract(prompt, into = "prompt", regex = "Q[0-9]+..(.*?)..Transcription") %>%
  rename("ResponseId" = responseId) %>% 
  right_join(ids, by = "ResponseId") %>% 
  filter(!is.na(text)) 
...
phonic = phonic %>% 
  left_join(prompt_text, by = "prompt") 


#### phonic data 
phonic = phonic %>% 
  select(pID, prompt, text, prompt_text)


###Join datasets together to do prelim analyses 
text = phonic %>% 
  select(-prompt_text) %>% 
  rbind(data_text) %>% 
  mutate(type = if_else(str_detect(prompt, "SWLS-Text|Autonomy-Text"), "Text", "Audio"),
    prompt = str_remove(prompt, "-Text"))


###join text data with behavioral data based on pid 
...
### spread text data to do embeddings 
...
write.csv(data,"/Users/stevenmesquiti/Downloads/wellbeing-prediction-text-long.csv")
...
## Get item level scores

```{r}
sona_wbp_items <- data_surveys %>% 
  select(pID, Reflection_Type, survey_name, item, value) %>% 
  filter(str_starts(item, "PWB_") | str_starts(item, "SWLS_")) %>% 
  mutate(value = if_else(item %in% c("PWB_4", "PWB_5", "PWB_6", "PWB_7", "PWB_10", "PWB_14", "PWB_15", "PWB_16"),
                         as.character(8 - as.numeric(value)), value)) %>% 
  mutate(value = as.numeric(value)) %>% 
  group_by(pID, Reflection_Type, survey_name, item) %>% 
...
rename(
    ...
    Autonomy_2 = PWB_17,
    Autonomy_3 = PWB_18,
    environmental_mastery_1 = PWB_4,
    environmental_mastery_2 = PWB_8,
    environmental_mastery_3 = PWB_9,
    personal_growth_1 = PWB_11,
    personal_growth_2 = PWB_12,
    personal_growth_3 = PWB_14,
    positive_relations_1 = PWB_6,
    positive_relations_2 = PWB_13,
    positive_relations_3 = PWB_16,
    purpose_1 = PWB_3,
    purpose_2 = PWB_7,
    purpose_3 = PWB_10,
    self_acceptance_1 = PWB_1,
    self_acceptance_2 = PWB_2,
    self_acceptance_3 = PWB_5
  )

write.csv(sona_wbp_items,"/Users/stevenmesquiti/Downloads/SONA_wellbeing-prediction-items.csv")
```
...
data_demo = data %>%
  ...
  filter(!grepl("phonic-practice|Click|Submit|Count|consent|screen|attention|therapy|SWLS|PWB|valence|personal|Autonomy|OE|reflection_useful_1|study_exp_helpful|feedback", item)) %>%
  select(id, pID, Reflection_Type,survey_name,item, value) 



ses_subjective = data_demo %>%
  filter(str_starts(item, "ses-ladder") & value == "On") %>% 
  mutate(scale_name = "ses-ladder") %>% 
    select(-value) %>%
  rename("item" = scale_name,
         "value" = item) %>% 
  select(pID, survey_name,item,value) %>% 
  mutate(value = str_replace(value, "ses-ladder_", ""))
...
demo = data_demo %>%
  filter(grepl("gender|age|income|practice|hispanic|feedback|first-|year|parental", item)) %>%
  mutate(value = case_when(
    ...
    grepl("transgender", item) ~ dplyr::recode(value, 
                                               ...
                                               "0" = "No"),
    grepl("year-college", item) ~ dplyr::recode(value, 
                                          "1" = "First-year",
                                          "2" = "Sophomore",
                                          "3" = "Junior",
                                          "4" = "Senior",
    ...
    grepl("income", item) ~ dplyr::recode(value, 
                                                       ...
                                                       "12" = "NA"),
        grepl("parental-edu", item) ~ dplyr::recode(value, 
                                          ...
                                          "6" = "Associate's degree (including occupational or 
                                          academic degrees)",
    ...
    grepl("first-gen", item) ~ dplyr::recode(value,
                                             "1" = "first-gen",
                                             "0" = "Not first gen",
                                             "2" = "Choose to not disclose"), TRUE ~ value
  )) %>% 
  bind_rows(., ses_subjective, ethnicity) %>%
  arrange(pID) %>% 
  select(-survey_name,-Reflection_Type,-id)
...
data_demo_unique <- demo %>%
  ...
  mutate(age = as.numeric(age),
         ...
         `parental-edu` = as.factor(`parental-edu`),
         `year-college` = as.factor(`year-college`))
