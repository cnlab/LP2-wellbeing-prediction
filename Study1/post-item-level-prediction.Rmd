...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, cache = FALSE,fig.path = "post_item_level/item_")
options(scipen=999)
```
...
```{r}
affect <- df %>%
  select(-post_SWLS_mean) %>% 
  select(starts_with("post_SWLS_"))

rds_file_path <- file.path(data_dir, "data_prediction/models/Study-1/Non-Content-Words/post-item-level/affect_item_output.RDS") #update with the path to each model
...
purpose <- df %>%
  select(-post_PWB_purpose) %>% 
  select(starts_with("post_Purpose_in_Life"))

rds_file_path <- file.path(data_dir, "data_prediction/models/Study-1/Non-Content-Words/post-item-level/purpose_item_level_output.RDS") #update with the path to each model
...
relationships <- df %>%
  select(-post_PWB_positive_relations) %>% 
  select(starts_with("post_Positive_Relations"))

rds_file_path <- file.path(data_dir, "data_prediction/models/Study-1/Non-Content-Words/post-item-level/relationships_item_level_output.RDS") #update with the path to each model
...
```{r}
acceptance <- df %>%
  select(-post_PWB_self_acceptance) %>% 
  select(starts_with("post_Self-Acceptance"))

rds_file_path <- file.path(data_dir, "data_prediction/models/Study-1/Non-Content-Words/post-item-level/acceptance_item_output.RDS") #update with the path
...
```{r}
growth <- df %>%
  select(-post_PWB_personal_growth) %>% 
  select(starts_with("post_Personal_Growth"))

rds_file_path <- file.path(data_dir, "data_prediction/models/Study-1/Non-Content-Words/post-item-level/growth_item_output.RDS") #update with the path
...
autonomy <- df %>%
  select(-post_PWB_autonomy) %>% 
  select(starts_with("post_Autonomy"))

rds_file_path <- file.path(data_dir, "data_prediction/models/Study-1/Non-Content-Words/post-item-level/autonomy_item_output.RDS") #update with the path
