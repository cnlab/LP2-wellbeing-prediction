...
df <- data %>%
  dplyr::select(1, starts_with("pre_")) %>%
  filter(rowSums(is.na(across(starts_with("pre_")))) == 0)
...
readr::write_csv(omega_results, "~/Desktop/LP2-wellbeing-prediction/Supplemental-Info/reliability/Study1_reliability_results.csv")
```
...
```{r}
affect <- df %>%
  dplyr::select(starts_with("pre_SWLS"))

rds_file_path <- file.path(data_dir, "supp/Study1/Item-level-analyses/affect_item_output.RDS") #update with the path to each model
...
purpose <- df %>%
  dplyr::select(starts_with("pre_Purpose_in_Life"))

rds_file_path <- file.path(data_dir, "supp/Study1/Item-level-analyses/purpose_item_level_output.RDS") #update with the path to each model model
...
```{r}
relationships <- df %>%
  dplyr::select(starts_with("pre_Positive_Relations"))

rds_file_path <- file.path(data_dir, "supp/Study1/Item-level-analyses/relationships_item_level_output.RDS") #update with the path to each model model
...
acceptance <- df %>%
  dplyr::select(starts_with("pre_Self.Acceptance")) %>%
  rename_with(~ gsub("\\.", "-", .), starts_with("pre_Self.Acceptance")) #replace period with _

rds_file_path <- file.path(data_dir, "supp/Study1/Item-level-analyses/acceptance_item_output.RDS") #update with the path to each model model
...
```{r}
growth <- df %>%
  dplyr::select(starts_with("pre_Personal_Growth"))

rds_file_path <- file.path(data_dir, "supp/Study1/Item-level-analyses/growth_item_output.RDS") #update with the path to each model model
...
autonomy <- df %>%
  dplyr::select(starts_with("pre_Autonomy"))

rds_file_path <- file.path(data_dir, "supp/Study1/Item-level-analyses/autonomy_item_output.RDS") #update with the path to each model model
