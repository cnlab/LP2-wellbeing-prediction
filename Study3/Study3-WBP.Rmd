...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, cache = FALSE,fig.path = "Study3_figs/fig_",dpi = 600)
options(scipen=999)
...
Sys.setenv(KMP_DUPLICATE_LIB_OK = "TRUE") #Temporarily allows execution despite duplicate OpenMP libraries bc with M series chips no other work around
library(text)
...
table <- function(correlation_data) {
  ...
  correlation_data$confidence_interval <- paste0("[", 
                                                 round(as.numeric(correlation_data$conf_int_low), 3), 
                                                 ", ", 
                                                 round(as.numeric(correlation_data$conf_int_high), 3), 
                                                 "]")
...
df = read.csv(file.path(data_dir,"data/WBP_Study3_Behavioral_Cleaned.csv"))
...
```{r}
items = read.csv(file.path(data_dir,"data/WBP_Study3_Behavioral_Cleaned_items.csv"))
...
readr::write_csv(omega_results, "~/Desktop/LP2-wellbeing-prediction/Supplemental-Info/reliability/Study3_reliability_results.csv")
...
```{r}
apa.cor.table(df[3:10],landscape = T,filename = file.path(data_dir,"Study3_correlation_matrix.doc"))
```
...
##extract information for heatmap - NOW INCLUDING CONFIDENCE INTERVALS
result_list_swls <- list(
  SWLS_acceptance = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB self_acceptance`$results,
  SWLS_autonomy = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB autonomy`$results,
  SWLS_growth = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB personal_growth`$results,
  SWLS_purpose = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB purpose`$results,
  SWLS_relationships = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB positive_relations`$results,
  SWLS_PWB = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB mean`$results,
  SWLS_SWLS = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_SWLS mean`$results,
  SWLS_mastery = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB environmental_mastery`$results
)

# Extract confidence intervals
SWLS_ci_data <- data.frame(
  model = names(result_list_swls),
  conf.int_low = sapply(result_list_swls, function(x) x$conf.int[1]),
  conf.int_high = sapply(result_list_swls, function(x) x$conf.int[2])
) %>%
  mutate(
    # Map the model names to match the outcome format in SWLS_subscale_results
    outcome = case_when(
      model == "SWLS_acceptance" ~ "PWB self_acceptance",
      model == "SWLS_autonomy" ~ "PWB autonomy",
      model == "SWLS_growth" ~ "PWB personal_growth",
      model == "SWLS_purpose" ~ "PWB purpose",
      model == "SWLS_relationships" ~ "PWB positive_relations",
      model == "SWLS_PWB" ~ "PWB mean",
      model == "SWLS_SWLS" ~ "SWLS mean",
      model == "SWLS_mastery" ~ "PWB environmental_mastery"
    )
  )
...
SWLS_subscale_results <- 
  ...
  mutate(
    ...
    outcome = ifelse(grepl("embeddings\\$texts\\$`SWLS-Text`_", descriptions), 
                     ...
                     NA)) |> 
  left_join(SWLS_ci_data %>% select(outcome, conf.int_low, conf.int_high), by = "outcome")
...
SWLS_heat <- ggplot(SWLS_subscale_results, aes(x = prompt, y = outcome, fill = correlation)) +
 geom_tile(color = "white") +
  geom_text(aes(label = paste0(round(correlation, 2), "(", round(disattenuated_correlation, 2), ")"),
          ...
          color = "black", size = 2.95) + 
  geom_text(aes(label = sprintf("95%%CI: [%.2f, %.2f]", conf.int_low, conf.int_high)), 
            color = "black", size = 2.5, vjust = 2) +  # Add CI below RMSE
  geom_text(aes(label = sprintf("RMSE: [%s]", round(RMSE, 3))), 
            color = "gray30", size = 2.5, vjust = 3.5) +
...
result_list_autonomy <- list(
  Autonomy_acceptance = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB self_acceptance`$results,
  Autonomy_autonomy = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB autonomy`$results,
  Autonomy_growth = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB personal_growth`$results,
  Autonomy_purpose = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB purpose`$results,
  Autonomy_relationships = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB positive_relations`$results,
  Autonomy_PWB = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB mean`$results,
  Autonomy_SWLS = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_SWLS mean`$results,
  Autonomy_mastery = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB environmental_mastery`$results
)

# Extract confidence intervals
Autonomy_ci_data <- data.frame(
  model = names(result_list_autonomy),
  conf.int_low = sapply(result_list_autonomy, function(x) x$conf.int[1]),
  conf.int_high = sapply(result_list_autonomy, function(x) x$conf.int[2])
) |> 
  mutate(
    # Map the model names to match the outcome format in Autonomy_subscale_results
    outcome = case_when(
      model == "Autonomy_acceptance" ~ "PWB self_acceptance",
      model == "Autonomy_autonomy" ~ "PWB autonomy",
      model == "Autonomy_growth" ~ "PWB personal_growth",
      model == "Autonomy_purpose" ~ "PWB purpose",
      model == "Autonomy_relationships" ~ "PWB positive_relations",
      model == "Autonomy_PWB" ~ "PWB mean",
      model == "Autonomy_SWLS" ~ "SWLS mean",
      model == "Autonomy_mastery" ~ "PWB environmental_mastery"
    )
  )
...
Autonomy_subscale_results <- 
  ...
  mutate(
    ...
    outcome = ifelse(grepl("embeddings\\$texts\\$`Autonomy-Text`_", descriptions), 
                     ...
                     NA)) |> 
  left_join(Autonomy_ci_data %>% select(outcome, conf.int_low, conf.int_high), by = "outcome")
...
Autonomy_heat <- ggplot(Autonomy_subscale_results, aes(x = prompt, y = outcome, fill = correlation)) +
  ...
  geom_text(aes(label = paste0(round(correlation, 2), "(", round(disattenuated_correlation, 2), ")"),
          ...
          color = "black", size = 2.95) + 
  geom_text(aes(label = sprintf("95%%CI: [%.2f, %.2f]", conf.int_low, conf.int_high)), 
            color = "black", size = 2.5, vjust = 2) +  # Add CI below correlation
  geom_text(aes(label = sprintf("RMSE: [%s]", round(RMSE, 3))), 
            color = "gray30", size = 2.5, vjust = 3.5) +  # Add RMSE below CI
  scale_fill_gradient2(low = "dodgerblue",
                       mid = "#FFFFCC",
                       high = "#c44536") +
...
result_list <- list(
  ...
  autonomy_mastery_sub = Autonomy_subscale$all_output$`embeddings$texts$\`Autonomy-Text\`_PWB environmental_mastery`$results
)
...
result_list <- list(
  ...
  SWLS_mastery_sub = SWLS_subscale$all_output$`embeddings$texts$\`SWLS-Text\`_PWB environmental_mastery`$results
...
saveRDS(all_effects,file.path(data_dir,"models/Study3/study3_all_effects.RDS"))
```
...
ggplot(comparison_df_shuffled, aes(x = correlation_difference)) +
  ...
  labs(title = "Distribution of Correlation Differences",
       ...
       y = "Density") +
  ggplot2::annotate("text", x = Inf, y = Inf, label = paste0("% greater than zero: ", round(percentage_greater_than_zero_shuffled, 2), "%"),
