...
setwd("/Users/stevenmesquiti/Desktop/LP2-within/LP2-intervention-within/Text-Prediction/Content-word-analysis/subscale-models/models/purpose")
##################purposehips######################################################################################

job::job({
  
####Pos Affect Predicting Autonomy############################################################################### 
  if (!file.exists("purpose_autonomy_content_sub.RDS")) {
    ...
    saveRDS(purpose_autonomy_sub, "purpose_autonomy_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    purpose_autonomy_sub <- readRDS("purpose_autonomy_content_sub.RDS")
  }
  
####purpose Predicting Purpose############################################################################### 
  if (!file.exists("purpose_purpose_content_sub.RDS")) {
    ...
    # Save the model output to an RDS file
    saveRDS(purpose_purpose_sub, "purpose_purpose_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    purpose_purpose_sub <- readRDS("purpose_purpose_content_sub.RDS")
  }
  
  ####purpose Predicting Relationships###############################################################################
  if (!file.exists("purpose_relationships_content_sub.RDS")) {
    ...
    # Save the model output to an RDS file
    saveRDS(purpose_relationships_sub, "purpose_relationships_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    purpose_relationships_sub <- readRDS("purpose_relationships_content_sub.RDS")
  }
  
  
  ####purpose Predicting Self-Acceptance############################################################################### 
  if (!file.exists("purpose_acceptance_content_sub.RDS")) {
    purpose_acceptance_sub <- textTrainRegression(
      x = embeddings$texts[10:12], # the three purpose prompts
      y = df$pre_PWB_self_acceptance, #predicting purpose 
      method_cor = "pearson",
    ...
    # Save the model output to an RDS file
    saveRDS(purpose_acceptance_sub, "purpose_acceptance_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    purpose_acceptance_sub <- readRDS("purpose_acceptance_content_sub.RDS")
  }
  
  ####purpose Personal Growth############################################################################### 
  if (!file.exists("purpose_growth_content_sub.RDS")) {
    purpose_growth_sub <- textTrainRegression(
      x = embeddings$texts[10:12], # the three purpose prompts
      y = df$pre_PWB_personal_growth, #predicting positive purpose 
      method_cor = "pearson",
    ...
    # Save the model output to an RDS file
    saveRDS(purpose_growth_sub, "purpose_growth_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    purpose_growth_sub <- readRDS("purpose_growth_content_sub.RDS")
  }
  
  ####purpose PWB############################################################################### 
  
  if (!file.exists("purpose_PWB_content_sub.RDS")) {
    ...
    # Save the model output to an RDS file
    saveRDS(purpose_PWB_sub, "purpose_PWB_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    purpose_PWB_sub <- readRDS("purpose_PWB_content_sub.RDS")
  }
  ...
  if (!file.exists("purpose_SWLS_content_sub.RDS")) {
    ...
    # Save the model output to an RDS file
    saveRDS(purpose_SWLS_sub, "purpose_SWLS_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    purpose_SWLS_sub <- readRDS("purpose_SWLS_content_sub.RDS")
  }
})
