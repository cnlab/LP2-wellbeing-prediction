...
setwd("/Users/stevenmesquiti/Desktop/LP2-within/LP2-intervention-within/Text-Prediction/Content-word-analysis/subscale-models/models/pos-affect")
##################Personal affect######################################################################################

####Pos Affect Predicting Autonomy############################################################################### 
if (!file.exists("affect_autonomy_content_sub.RDS")) {
  ...
  saveRDS(affect_autonomy_sub, "affect_autonomy_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  affect_autonomy_sub <- readRDS("affect_autonomy_content_sub.RDS")
}

####affect Predicting Purpose############################################################################### 
if (!file.exists("affect_purpose_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(affect_purpose_sub, "affect_purpose_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  affect_purpose_sub <- readRDS("affect_purpose_content_sub.RDS")
}

####affect Predicting Relationships###############################################################################
if (!file.exists("affect_relationships_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(affect_relationships_sub, "affect_relationships_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  affect_relationships_sub <- readRDS("affect_relationships_content_sub.RDS")
}


####affect Predicting Self-Acceptance############################################################################### 
if (!file.exists("affect_acceptance_content_sub.RDS")) {
  affect_acceptance_sub <- textTrainRegression(
    x = embeddings$texts[7:9], # the three affect prompts
    y = df$pre_PWB_self_acceptance, #predicting affect 
    method_cor = "pearson",
  ...
  # Save the model output to an RDS file
  saveRDS(affect_acceptance_sub, "affect_acceptance_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  affect_acceptance_sub <- readRDS("affect_acceptance_content_sub.RDS")
}

####affect Personal Growth############################################################################### 
if (!file.exists("affect_growth_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(affect_growth_sub, "affect_growth_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  affect_growth_sub <- readRDS("affect_growth_content_sub.RDS")
}

####affect PWB############################################################################### 
job::job({

if (!file.exists("affect_PWB_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(affect_PWB_sub, "affect_PWB_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  affect_PWB_sub.RDS <- readRDS("affect_PWB_content_sub.RDS")
}
...
job::job({
if (!file.exists("affect_SWLS_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(affect_SWLS_sub, "affect_SWLS_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  affect_SWLS_sub <- readRDS("affect_SWLS_content_sub.RDS")
}
})
