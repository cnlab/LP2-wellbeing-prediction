...
setwd("/Users/stevenmesquiti/Desktop/LP2-within/LP2-intervention-within/Text-Prediction/Content-word-analysis/subscale-models/models/relations")
##################relationships######################################################################################
...
if (!file.exists("relations_autonomy_content_sub.RDS")) {
  ...
  saveRDS(relations_autonomy_sub, "relations_autonomy_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  relations_autonomy_sub <- readRDS("relations_autonomy_content_sub.RDS")
}

####relations Predicting Purpose############################################################################### 
if (!file.exists("relations_purpose_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(relations_purpose_sub, "relations_purpose_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  relations_purpose_sub <- readRDS("relations_purpose_content_sub.RDS")
}

####relations Predicting Relationships###############################################################################
if (!file.exists("relations_relationships_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(relations_relationships_sub, "relations_relationships_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  relations_relationships_sub <- readRDS("relations_relationships_content_sub.RDS")
}


####relations Predicting Self-Acceptance############################################################################### 
if (!file.exists("relations_acceptance_content_sub.RDS")) {
  relations_acceptance_sub <- textTrainRegression(
    x = embeddings$texts[13:15], # the three relations prompts
    y = df$pre_PWB_self_acceptance, #predicting relations 
    method_cor = "pearson",
  ...
  # Save the model output to an RDS file
  saveRDS(relations_acceptance_sub, "relations_acceptance_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  relations_acceptance_sub <- readRDS("relations_acceptance_content_sub.RDS")
}

####relations Personal Growth############################################################################### 
if (!file.exists("relations_growth_content_sub.RDS")) {
  ...
  # Save the model output to an RDS file
  saveRDS(relations_growth_sub, "relations_growth_content_sub.RDS")
} else {
  # If the RDS file already exists, load the data from it
  relations_growth_sub <- readRDS("relations_growth_content_sub.RDS")
}

####relations PWB############################################################################### 
  
  if (!file.exists("relations_PWB_content_sub.RDS")) {
    ...
    # Save the model output to an RDS file
    saveRDS(relations_PWB_sub, "relations_PWB_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    relations_PWB_sub.RDS <- readRDS("relations_PWB_content_sub.RDS")
  }

####relations SWLS############################################################################### 


  if (!file.exists("relations_SWLS_content_sub.RDS")) {
    ...
    # Save the model output to an RDS file
    saveRDS(relations_SWLS_sub, "relations_SWLS_content_sub.RDS")
  } else {
    # If the RDS file already exists, load the data from it
    relations_SWLS_sub <- readRDS("relations_SWLS_content_sub.RDS")
  }
})
