...
#plot
SWLS_SWLS_study3 = ggplot(sig_high_low) +
  geom_point(aes(x_plotted, y = 0 , color = with_without), size = 0.5) +
  geom_text_repel(
    aes(
      x_plotted, y = 0 * sample(c(-0.7, 0.7), nrow(sig_high_low), replace = TRUE),
...
plot_list <- list(SWLS_SWLS_study3, SWLS_accpt, SWLS_purpose, SWLS_relations,
                   SWLS_growth, SWLS_autonomy, SWLS_pwb, SWLS_mastery)
...
study3plot_list <- list(autonomy_autonomy, SWLS_SWLS_study3)

study3combined_plot <- ggarrange(plotlist = study3plot_list,
                           ncol = 1, nrow = 2,
                           labels = letters[1:length(plot_list)],
                           label.x = 0.05,   # optional: adjust label position
                           label.y = 0.95,
...
study3combined_plot_with_title <- annotate_figure(
  study3combined_plot,
  top = text_grob(
...
study3combined_plot_with_title
```
