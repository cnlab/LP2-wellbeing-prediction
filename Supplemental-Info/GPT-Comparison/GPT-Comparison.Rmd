...
#rename the ML model confidence interval columns for consistency
bert_autonomy <- bert_autonomy %>%
  rename(
    CI_lower = `conf.int_low`,
    CI_upper = `conf.int_high`
  )
...
facet_data <- model_dfs %>%
  imap_dfr(~ .x %>%
    mutate(Model = .y) %>%
    select(prompt, outcome, correlation, disattenuated_correlation, RMSE, p_value_corrected, Model,CI_lower,CI_upper) %>%
